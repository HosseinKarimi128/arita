...
def last_message_with_url(file_description: dict, prompt: str):
...
async def receive_message(request: MessageRequest):
    ...
    if request.url:
        ...
        request.url, request.content_type = update_url(request.url)
        description = describe_media(request, formatted_history)
        last_human_message = last_message_with_url(description, request.content)

    
    response = chatbot.generate_prompt(user_request=last_human_message,
